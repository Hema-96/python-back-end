...
class AuthService:
    ...
    def register_user(self, user_data: UserRegister) -> Dict[str, Any]:
        try:
            ...
            return {"user": {"id": user.id, "email": user.email, "first_name": user.first_name, "last_name": user.last_name, "role": user.role, "is_active": user.is_active, "is_verified": user.is_verified}, "tokens": tokens}
    ...
    def create_college_profile(self, user_id: int, profile_data: CollegeProfileCreate) -> CollegeProfile:
        try:
            ...
            if not user or user.role != UserRole.COLLEGE:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User not found or not a college")
            ...
            if existing_college:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="College code already exists")
    ...
    def create_admin_profile(self, user_id: int, profile_data: AdminProfileCreate) -> AdminProfile:
        try:
            ...
            if not user or user.role != UserRole.ADMIN:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User not found or not an admin")
    ...
    def create_student_profile(self, user_id: int, profile_data: StudentProfileCreate) -> StudentProfile:
        try:
            ...
            if not user or user.role != UserRole.STUDENT:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="User not found or not a student")
